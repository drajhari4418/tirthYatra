...
const serviceLinkMap = {"shop":"prasads.html","live pujas":"temple-visits.html"};
const normalizeService = s => {
  const title = String(s.title || "").trim(), key = title.toLowerCase();
  if (key === "tirth yatra") return null;
  const fallback = defaultServices.find(x => x.title.toLowerCase() === key);
  return {...s,title:fallback?.title||title,image:fallback?.image||s.image||"images/service1.jfif",link:serviceLinkMap[key]||s.link||fallback?.link||"#"};
};
...
export function bindServices(target = document.getElementById("dynamic-services")) {
  if (!target) return () => {};
  return onValue(ref(db, "services"), snap => {
    const value = snap.val() || {};
    const items = Object.entries(value).map(([id,data]) => ({id,...(data||{})})).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0)).slice(0,50);
